...
```{r project.setup, results = FALSE, message = FALSE, warning=FALSE}
#Import libraries
library(tidyverse)
library(lubridate)
library(here)
...
epa_data <- read.csv(
  here("Data","Raw", "Processed_KEY","EPAair_O3_PM25_NC1819_Processed.csv"),
...
```{r, warning = FALSE,echo = FALSE, fig.align='left', fig.height=3.5, fig.cap='Distribution of Ozone levels by month in 2018'}
ozone2018 <-ggplot(filter(epa_data, Year == 2018), 
...
```{r, warning = FALSE,echo = FALSE, fig.align='left', fig.height=3.5,fig.cap='Distribution of Ozone levels by month in 2019'}
ozone2019 <-ggplot(filter(epa_data, Year == 2019), 
                   aes(x=factor(Month), y=Ozone))+
                  labs(x= "Month")+
  geom_boxplot()
print(ozone2019)
